	...
	public double handleCompareOp(IModelInstanceObject env,
		...
		if (left.getClass().getSimpleName().equals("JavaOclReal")
			...
			double leftResult_beforeDecimal = Double.valueOf(leftResult
					.toString().split("\\.")[0]);
			double leftResult_afterDecimal = getReviseValue(Double.valueOf(leftResult
					.toString().split("\\.")[1]));
//			double leftResult_afterDecimal = Utility.INSTANCE
//					.formatRealValueWithoutZero(leftResult.toString().split(
//							"\\.")[1]);
			double rightResult_beforeDecimal = Double.valueOf(rightResult
					.toString().split("\\.")[0]);
			double rightResult_afterDecimal = getReviseValue(Double.valueOf(rightResult
					.toString().split("\\.")[1]));
	...
	//Low filling
	// add by luhong
	private double getReviseValue(double value)
	{
		...
		for(int i = strsize; i <=10; i++)
		{
			valuestr = valuestr.concat("0");
			sb.append("0");
		}
	...
	public double handleComplexSelectSizeOp(IModelInstanceObject env,
		...
		if (opName.equals(">") || opName.equals(">=")) {
			...
			if ((leftResult - rightResult) > 0)
				...
				//changed by luhong :selfsize to leftresult
