	...
	public int buildIndexArray4Bound(Constraint constraint) {
		...
		while (it.hasNext()) {
			...
			if (e instanceof OperationCallExpImpl) {
				...
				if (oce.getReferredOperation().getName().equals("and")) {
					OperationCallExpImpl leftExp = (OperationCallExpImpl) oce
							.eContents().get(0);
					OperationCallExpImpl rightExp = (OperationCallExpImpl) oce
							.eContents().get(1);
					if (isBelongToOp(leftExp.getReferredOperation().getName(),
							this.OP_COMPARE)
							&& isBelongToOp(rightExp.getReferredOperation()
									.getName(), this.OP_COMPARE)) {
//						if (hasSameOpPart(leftExp, rightExp)) {
//							oce.getReferredOperation().setName("or");
////							oce.setReferredOperation(newReferredOperation);
////							new OperationImpl ();
//						}
					}

				} else if (isBelongToOp(oce.getReferredOperation().getName(),
						this.OP_COMPARE)) {
...
public boolean hasSameOpPart(OclExpression exp1, OclExpression exp2){
		...
		if("= <> < <= >>=".indexOf(((OperationCallExpImpl)exp1).getReferredOperation().getName()) < 0 || 
				"= <> < <= >>=".indexOf(((OperationCallExpImpl)exp2).getReferredOperation().getName()) < 0){
			return false;
		}
